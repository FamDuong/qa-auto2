...
from config.environment import COCCOC_POINTS_REDIS_HOST
from config.environment import COCCOC_POINTS_REDIS_PORT
from config.environment import COCCOC_POINTS_REDIS_PASSWORD

import logging
LOGGER = logging.getLogger(__name__)

class CocCocPointsRedis:
    # Enable cluster mode
    def redis_cli_init_cluster(self):
        init_host = COCCOC_POINTS_REDIS_HOST.replace("{i}", "0")
        startup_nodes = [{"host": init_host, "port": COCCOC_POINTS_REDIS_PORT}]
        redis_cli = RedisCluster(startup_nodes=startup_nodes, decode_responses=True, password=COCCOC_POINTS_REDIS_PASSWORD, skip_full_coverage_check=True)
        redis_cli.ping()
        return redis_cli
    ...
    def redis_cli_init_strict(self, node = 0):
        ...
        redis_cli = redis.StrictRedis(host=COCCOC_POINTS_REDIS_HOST.replace("{i}", str(node)), port=COCCOC_POINTS_REDIS_PORT, db=0,
                                      password=COCCOC_POINTS_REDIS_PASSWORD)
        return redis_cli
    ...
    def redis_get(self, query, format_data ="json"):
        ...
        try:
            ...
            if format_data == "json" and data != None:
    ...
    # Delete a key
    def redis_del(self, query):
        try:
            redis_cli = self.redis_cli_init_cluster()
            # redis_cli.ping()
            data = redis_cli.delete(query)
        except Exception as e:
            print(e)
            exit('Failed to connect, terminating.')
        finally:
