...
class TestFrontendApi:
    ...
    def test_get_user_feed_setting(self, coccoc_new_feeds_db_interact):
        ...
        for vid in list_db_vid:
            ...
            list_api_sub_category_id = api_data['sub_categories']
            # list_api_sub_category_name = self.new_feed_api.get_list_json_level_1(api_data['sub_categories'], "name")
            list_api_block_sources_domain = self.common.get_list_json_level_1(api_data['block_sources'], "domain")
            list_api_block_sources_title = self.common.get_list_json_level_1(api_data['block_sources'], "title")
            ...
            result_tmp = self.common.check_if_element_different_in_lists(list_api_sub_category_id, list_db_sub_category_id)
            ...
            #    result = False
            result_tmp = self.common.check_if_element_different_in_lists(list_api_block_sources_domain, list_db_block_sources_domain)
            if result_tmp is False:
                ...
                result = False
            result_tmp = self.common.check_if_element_different_in_lists(list_api_block_sources_title, list_db_block_sources_title)
            if result_tmp is False:
                print("    ERROR: Block source domains are incorrect")
                print("    api:", list_api_block_sources_title)
                print("    db :", list_db_block_sources_title)
                result = False
            # NF-176: Check all title are UPPER CASE
            list_title_islower = [word for word in list_api_block_sources_title if word.islower()]
            if len(list_title_islower) > 0:
                print("    ERROR: Titles are not UPPER")
                print(list_title_islower)
                result = False

        assert result == True

    # NF-78	[Frontend API] Get user feed setting - if vid not exist
    def test_get_new_user_feed_setting(self, coccoc_new_feeds_db_interact):
        result = True
        # Generate a new vid
