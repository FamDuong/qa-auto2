...
class TestCmsApi:
    ...
    def test_get_list_block_article(self, coccoc_new_feeds_db_interact):
        ...
        db_data = self.new_feed_db.get_newfeeds_db_connection(coccoc_new_feeds_db_interact, f'select article_id from user_complaint_articles where status = "block" and update_time > date_sub(now(), interval 3 day) and now();')
