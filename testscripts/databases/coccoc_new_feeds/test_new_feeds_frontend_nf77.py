...
class TestFrontendApi:
    ...
    def test_get_categories(self, coccoc_new_feeds_db_interact):
        ...
        for group in list_api_groups:
            print("Group: ", group)
            list_api_categories = self.new_feed_api.get_list_json_level_2(api_categories, 'group', group, 'categories')

            db_category = self.new_feed_db.get_newfeeds_db(coccoc_new_feeds_db_interact,
                                                           f'SELECT category_id , name,	image_url FROM categories WHERE	`group` = "{group}" AND status = "active" order by `order`;')
            # Compare ID
            list_api_categories_id = self.new_feed_api.get_list_json_level_1(list_api_categories, 'id')
            ...
            # Compare name
            list_api_categories_name = self.new_feed_api.get_list_json_level_1(list_api_categories, 'name')
            ...
            # Compare url image
            list_api_categories_image_url = self.new_feed_api.get_list_json_level_1(list_api_categories, 'image_url')
