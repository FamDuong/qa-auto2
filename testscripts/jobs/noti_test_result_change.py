...
from enums.jira import JiraIssueStatus
    ...
    for each_test_id in test_id_result_change_list:
        ...
        for each in TestResultStatus:
            ...
            if new_status == each.value:
                new_status_name = each.name
        message_content_to_send += f"Test case {test_id_title} has been changed from {old_status_name} to {new_status_name} \n"
        if new_status_name == 'FAILED':
            jira_issue_id = test_rail_utils.get_test_id_info(test_id=each_test_id).get('refs')
            if jira_utils.get_issue_status(issue_id=jira_issue_id) == JiraIssueStatus.DONE.name:
                jira_utils.add_comment_jira(issue_id=jira_issue_id, comment_content=message_content_to_send)
                jira_utils.change_status(issue_id=jira_issue_id, status=JiraIssueStatus.BACK_LOG.value)
